...
export default class CreativeOption extends React.Component
{
    ...
    async getItems (params = {})
    {
        ...
        let res = await fetchData('/getOptions?' + query);
    ...
    edit(data, row)
    {
        this.setState({visible: true, title: 'Update Creative Option', buttonText: 'Update', option: data});
    }

    Add()
    {
        this.setState({visible: true, title: 'Add Creative Option',  buttonText: 'Add', option:{name: '', creativeSection: {_id: '', name: ''}}});
    }
    ...
    async process()
    {
        ...
        let res = await postQuery(url, JSON.stringify({_id: this.state.option._id, name: this.state.option.name}));
                ...
                <div  className="main-content" id="mainContent">
                    <div className="custom-filter-dropdown">
                        <Row style={{margin: '20px'}}>
                            <Col span="24">
                                <h4 style={{fontWeight: 'bold', fontSize: '18px'}}>Creative Options</h4>
                            </Col>
                        </Row>
                        ...
                        >
                            <Form>
                                <div id="singleEntryForm">
                                    <Item>
                                        <input onChange={this.textChange} name="name" type="text" className="ant-input ant-input-lg input-no-border" placeholder="Creative Option Name" value={option.name}/>
                                    </Item>
                                </div>
                            </Form>
                        </Modal>
