...
export const fetchData = async (path, jwToken) =>
  fetch(`http://localhost:80${path}`,
  {
      method: "GET",
      headers:
      {
        Accept: 'application/json', 'Content-Type': 'application/json', Authorization: `Bearer ${jwToken || ''}`
      }
  }).then(data => data.json());
...
export const postQuery = async (path, body, jwToken) =>
    fetch(`http://localhost:80${path}`,
        {
            method: "POST",
            headers:
